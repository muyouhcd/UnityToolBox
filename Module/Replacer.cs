...
namespace DYM.ToolBox
{
    ...
    public class Replacer : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("转换为嵌套Prefab"))
            {
                GenerateNestedPrefabs();
            }
        ...
        void GenerateNestedPrefabs()
        {
            ...
            foreach (GameObject obj in Selection.gameObjects)
            {
                ...
                {
                    Debug.Log($"Object {obj.name} is already a prefab instance. Creating nested reference...");

                    // 使用原始预制体的名称
                    string prefabName = sourcePrefab.name;
                    string localPath = $"{prefabPath}/{prefabName}.prefab";

                    // 检查是否已存在同名预制体
                    if (System.IO.File.Exists(localPath))
                    {
                        Debug.LogWarning($"Nested prefab already exists: {localPath}");
                        continue;
                    }

                    // 创建一个空的父物体
                    GameObject parentObject = new GameObject(prefabName + "_Container");
                    ...
                    // 保存为嵌套预制体
                    GameObject newPrefab = PrefabUtility.SaveAsPrefabAsset(parentObject, localPath);
                    if (newPrefab != null)
                    {
                        Debug.Log($"Nested prefab created: {localPath}");

                        // 用新的嵌套预制体替换场景中的物体
                        GameObject newInstance = (GameObject)PrefabUtility.InstantiatePrefab(newPrefab);
                        newInstance.transform.SetParent(obj.transform.parent);
                    ...
                    {
                        Debug.LogError($"Failed to create nested prefab: {localPath}");
                        DestroyImmediate(parentObject);
                    }
                }
                else
                {
                    // 如果物体不是预制体实例，检查是否有MeshRenderer或其他组件
                    ...
                    if (meshRenderer != null && meshFilter != null && meshFilter.sharedMesh != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(meshPath))
                        {
                            ...
                            {
                                // 使用Mesh资产的名称
                                string assetName = meshAsset.name;
                                string localPath = $"{prefabPath}/{assetName}.prefab";
                                ...
                                // 创建一个空的父物体
                                GameObject parentObject = new GameObject(assetName + "_Container");

                                // 实例化Mesh资产作为子物体
                                GameObject meshInstance = (GameObject)PrefabUtility.InstantiatePrefab(meshAsset);
                                meshInstance.transform.SetParent(parentObject.transform);
                                ...
                                if (newPrefab != null)
                                {
                                    Debug.Log($"Nested prefab created from mesh asset: {localPath}");
                            ...
                            {
                                Debug.LogWarning($"Could not load mesh asset from path: {meshPath}");
                            }
                        }
                        else
                        {
                            Debug.LogWarning($"Mesh asset path is empty for object: {obj.name}");
                        }
                    }
                    else
                    {
                        Debug.LogWarning($"Object {obj.name} does not have MeshRenderer/MeshFilter or is not a prefab instance. Skipping...");
                    }
                }
            }
