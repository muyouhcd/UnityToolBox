...
namespace DYM.ToolBox
{
    ...
    public class RenamePlus : EditorWindow
    {
        ...
        private Dictionary<string, string> ParseCsvForRename(string csvPath)
        {
            ...
            lines.Skip(1)
                ...
                .ForEach(tokens =>
                {
                    if (tokens.Length >= 3)
                    {
                        ...
                        // 跳过无效GUID的资产
                        if (guid == "NOT_UNITY_ASSET" || guid == "错误")
                        {
                            skippedCount++;
                            return;
                        }
            ...
            if (skippedCount > 0)
            {
                Debug.LogWarning($"从CSV文件中跳过 {skippedCount} 个非Unity资产（标记为NOT_UNITY_ASSET或错误）。这些资产将尝试通过路径方式重命名。");
            }
        ...
        private void ExportAssetsCsv()
        {
            ...
            try
            {
                ...
                int unityAssetCount = 0;
                int nonUnityAssetCount = 0;
                List<string> nonUnityAssetPaths = new List<string>();
                ...
                assetPath =>
                {
                    try
                    {
                        ...
                        // 如果无法获取GUID，生成一个明确的标记
                        if (string.IsNullOrEmpty(guid))
                        {
                            // 使用一个明确的前缀标识这不是真正的Unity GUID
                            guid = "NOT_UNITY_ASSET";
                            nonUnityAssetCount++;
                            nonUnityAssetPaths.Add(assetPath);
                        }
                ...
                Debug.Log($"统计: 共处理 {assetPaths.Count} 个资源, 其中 {unityAssetCount} 个是Unity资源(有真实GUID), {nonUnityAssetCount} 个是非Unity资源(没有真实GUID)");

                if (nonUnityAssetCount > 0)
                {
                    Debug.LogWarning($"警告: 有 {nonUnityAssetCount} 个资源不是Unity资产或不在Unity项目中，这些资产的GUID标记为 'NOT_UNITY_ASSET'，使用CSV重命名时将无法通过GUID方式重命名。");
                    if (nonUnityAssetCount <= 20)
                    {
                        Debug.LogWarning($"非Unity资产路径列表:\n{string.Join("\n", nonUnityAssetPaths)}");
                    }
                    else
                    {
                        Debug.LogWarning($"非Unity资产过多，只显示前20个:\n{string.Join("\n", nonUnityAssetPaths.Take(20))}\n...以及更多{nonUnityAssetCount - 20}个");
                    }
                }
            }
            catch (Exception ex)
